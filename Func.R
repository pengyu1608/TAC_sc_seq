...
preTSNE<- function(input,cell_info,name,var.genes=c(),ifNorm=TRUE,min.cell=10,min.gene=500,mito.perc=1,x.cutoff=3,y.cutoff=1,regress_factor=c(),doJackStraw=FALSE){
	...
	if(length(var.genes) !=0){
	
		var.genes<-intersect(var.genes,rownames(data@data))
		data@var.genes<-var.genes
	}
...
create_cell_umi_matrix_mod <-function(umi_file,gene_info_file,prefix,mod_info_matrix=c()){
	...
	cell_id<-as.character(toupper(cell_id))
	...
	merge_gene_info$ID=as.character(merge_gene_info$ID)
...
draw_stack_perc<- function(x,column,mod_string=c(),orderBy=c(),class_level=c()){
...
#		}
		...
		if(!is.null(class_level)){
			...
			class_percent$class<-factor(class_percent$class,levels=class_level)
			group_percent$class<-factor(group_percent$class,levels=class_level)
		...
		#p2_2<-ggplot(class_percent,aes(x=cluster,y=percent,label=paste(percent,"%"),fill=class))+ geom_bar(stat="identity",position=position_dodge())+coord_flip()
		p2_2<-ggplot(class_percent,aes(x=class,y=percent,fill=class,label=paste(percent,"%")))+ geom_bar(stat="identity",position=position_dodge())+facet_wrap( ~ cluster, ncol=5,scales="free")
...
write_marker<-function(x,file,mod_string=c(),orderBy=c(),class_level=c()){
...
#	plot_obj$data$group<-sub("_.*","",plot_obj$data$group)
	...
	draw_stack_perc(plot_obj,"condition",mod_string,orderBy=orderBy,class_level=class_level)
...
draw_tsne<- function (tsne_obj,color_set){
	data_embeding<-data.frame(tsne_obj$seurat@dr$tsne@cell.embeddings,Cluster=tsne_obj$seurat@ident)
	p<-ggplot(data_embeding,aes(x=tSNE_1,y=tSNE_2,colour=Cluster))+scale_colour_brewer(palette=color_set)+geom_point()+xlab("tSNE 1")+ylab("tSNE 2")
	p<-font_theme(p)
	return(p)
